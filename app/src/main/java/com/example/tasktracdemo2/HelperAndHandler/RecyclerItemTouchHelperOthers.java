...
public class RecyclerItemTouchHelperOthers extends ItemTouchHelper.SimpleCallback {
    //define the global variables
    private OthersAdapter adapter;

    //constructor for the class
    public RecyclerItemTouchHelperOthers (OthersAdapter adapter){
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (direction == ItemTouchHelper.LEFT){
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(adapter.getContextOthers()); // alert prompt
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if(dX>0){
            icon = ContextCompat.getDrawable(adapter.getContextOthers(), R.drawable.swipe_edit);
            background = new ColorDrawable(ContextCompat.getColor(adapter.getContextOthers(), R.color.purple_200));
        } else {
            icon = ContextCompat.getDrawable(adapter.getContextOthers(), R.drawable.swipe_delete);
